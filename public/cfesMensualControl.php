...
foreach ($arrayCompanies as $rut) {
	...
	$vouchersList = file_get_contents(URL_REST.'company/'.$rut.'/cfe/emitidos?From='.$from.'&To='.$to.'&PageSize='.$limit, false, $contexto);
	...
	foreach ($vouchersList as $voucher) {
		...
		switch ($voucher->tipoCFE) {
			...
			default: echo nl2br($voucher->id." no se reconoce tipoCFE ".$voucher->tipoCFE."\n");
		}
	}
	...
	foreach ($arrayCountVouchers as $tipo => $cantidad) {

		if ( $cantidad > 0 ){
			$sql = "INSERT INTO `uso_cfes` (`rut`, `tipocfe`, `cantidadUsados`, `periodo`) VALUES ($rut, $tipo, $cantidad, '202209')";
